...
2. The **Pokédex web app** at the **repository root** (`src/`, `scripts/`, `public/`, `index.html`, `package.json`, `vite.config.ts`, …) — a React + Vite + TypeScript app, built test-first from its spec (posted as a comment on [issue #1](https://github.com/klemieux-octopus/claude-pokedex/issues/1) — GitHub Issues are the source of truth). Unlike the skills, it *does* have a build, a test suite, and a GitHub Pages deploy; see [`README.md`](README.md) for the dev / test / build / deploy commands.
...
- **Always run an end-to-end test after the code review, before committing.** The sequence when building is `/implement` → `/tdd` → `/code-review` → **test de bout en bout** → commit. Unit tests passing is not enough: once the review is clean, launch the actual app and drive it as a user would (`/run` — pour la Pokédex, l'app dans un navigateur) to confirm the change really works. Only commit once that end-to-end check passes.
